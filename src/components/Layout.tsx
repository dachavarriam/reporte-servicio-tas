import { BarChart3, Bell, Clock3, FileText, Home, LogOut, Menu, Plus, UserRound, Users, X } from 'lucide-react';
import { useState } from 'react';
import { NavLink, Outlet, useNavigate } from 'react-router-dom';
import { useApp } from '../store/app';

const nav = [
  { to: '/', label: 'Reportes', icon: Home }, { to: '/pendientes', label: 'Pendientes', icon: Clock3 },
  { to: '/metricas', label: 'Métricas', icon: BarChart3 }, { to: '/usuarios', label: 'Usuarios', icon: Users }, { to: '/perfil', label: 'Perfil', icon: UserRound }
];
...
export function Layout() {
  ...
  return <div className="shell">
    <aside className={`sidebar ${menu ? 'open' : ''}`}>
      ...
      <button className="btn btn-primary create-sidebar" onClick={() => { navTo('/nuevo'); setMenu(false); }}><Plus size={18} /> Crear nuevo reporte</button>
      <nav>{nav.map(({ to, label, icon: Icon }) => <NavLink key={to} to={to} end={to === '/'} onClick={() => setMenu(false)}><Icon size={20} />{label}</NavLink>)}</nav>
    ...
    <div className="app-area">
      ...
      <main><Outlet /></main>
      <nav className="bottom-nav"><NavLink to="/" end><Home /><span>Reportes</span></NavLink><NavLink to="/pendientes"><Clock3 /><span>Pendientes</span></NavLink><button onClick={() => navTo('/nuevo')}><Plus /></button><NavLink to="/perfil"><UserRound /><span>Perfil</span></NavLink></nav>
    </div>
